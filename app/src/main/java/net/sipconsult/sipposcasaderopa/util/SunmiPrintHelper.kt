...
//                sunmiPrinterService!!.printText("\n纵向排列\n", null)
     ...
     * result)
     */
    fun printTrans(context: Context, callbcak: InnerResultCallbcak?) {
        if (sunmiPrinterService == null) {
            //TODO Service disconnection processing
            return
        }
     ...
     */
    fun sendTextToLcd(text: String) {
        if (sunmiPrinterService == null) {
            //TODO Service disconnection processing
            return
        }
        try {
            sunmiPrinterService!!.sendLCDFillString(text, 16, true, object : InnerLcdCallback() {
    ...
    fun printExample(context: Context) {
        ...
        try {
            val paper = sunmiPrinterService!!.printerPaper
            sunmiPrinterService!!.printerInit(null)
            sunmiPrinterService!!.setAlignment(1, null)
            sunmiPrinterService!!.printText("测试样张\n", null)
            val bitmap =
                BitmapFactory.decodeResource(context.resources, R.drawable.company_logo)
            sunmiPrinterService!!.printBitmap(bitmap, null)
            ...
            try {
                ...
                sunmiPrinterService!!.sendRAWData(byteArrayOf(0x1B, 0x33, 0x00), null)
            }
            ...
            sunmiPrinterService!!.printTextWithFont("总计:          59¥\b", null, 40f, null)
            sunmiPrinterService!!.setAlignment(1, null)
